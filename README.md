...
텍스트 검수 중 `찾기·바꾸기`를 열면 여러 댓글 블록을 하나의 문서처럼 검색할 수 있습니다. `Ctrl+F`는 찾기, `Ctrl+H`는 바꾸기, `F3`과 `Shift+F3`은 다음·이전 결과로 이동합니다. 일괄변경 직후에는 `변경 되돌리기`로 한 번 복원할 수 있습니다. `저장 자막 보기` 창에서도 `Ctrl+F`로 대사를 찾고 `Enter`·`F3`은 다음, `Shift+Enter`·`Shift+F3`은 이전 결과로 이동하며 `Esc`로 찾기 바를 닫습니다.
...
- 목록 새로고침: 입력한 API 키로 지금 쓸 수 있는 텍스트 모델 목록을 Google에서 받아와 선택
- 모델 상태 점검: 목록의 모델(최대 12개)에 짧은 요청을 한 번씩 보내 정상·과부하(503)·은퇴(404)와 응답 시간을 표시
